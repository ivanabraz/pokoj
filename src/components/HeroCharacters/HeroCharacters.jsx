...
const HeroCharacters = ({HeroCharactersImgs}) => {
    ...
    return (
        <div id="characters" className="flex flex-row flex-wrap w-100 h-[auto] h-fit items-center justify-center overflow-hidden py-20">
            {HeroCharactersImgs.map((characters) => (
                <div key={characters.id} className='justify-center text-center mx-20 my-1 transition ease-in-out hover:scale-105 duration-300'>
                    <Link to={`#${characters.link}`}>
                        <img className="w-[70%] xs:w-[70%] sm:w-[70%] md:w-[80%] lg:w-[80%] xl:w-[90%] 2xl:w-[90%] h-auto m-auto" src={characters.img} alt={characters.id}/>
